				...
				<div class="col-md-7 main" id="contentPart">
					<form action="createQuestion.do" method="post">
						<fieldset>
							...
							<div class="form-group">
								<label>標題：</label><input class="form-control" type="text"
									name="qtitle" style="width: 100%"> &nbsp;<small><font
									color="red" size="-3">${errors.errorTitle}</font></small>
							</div>
							<label>內容：</label>
					...
					<c:if test="${!empty questionSuccess || !empty quesstionFailed }">
						<div id="dialog" title="${questionSuccess}${quesstionFailed}">
							<H3>
								<Font color='red'>${questionSuccess}</Font>
							</H3>
							<H3>
								<Font color='red'>${quesstionFailed}</Font>
							</H3>
						</div>
						<script>
							$(function() {
